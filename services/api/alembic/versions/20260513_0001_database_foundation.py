...
def upgrade() -> None:
    ...
    op.execute(
        ...
        CREATE TABLE content.articles (
            ...
            CONSTRAINT ck_articles_status CHECK (status IN ('draft', 'review', 'published', 'archived')),
            CONSTRAINT fk_articles_author_id_users FOREIGN KEY (author_id) REFERENCES auth.users(id)
        );
        ...
        CREATE TABLE content.article_blocks (
            ...
            CONSTRAINT fk_article_blocks_article_id_articles FOREIGN KEY (article_id) REFERENCES content.articles(id) ON DELETE CASCADE
        );
        ...
        CREATE TABLE stats.players (
            ...
            CONSTRAINT ck_players_handedness CHECK (handedness IS NULL OR handedness IN ('left', 'right'))
        );
        ...
        CREATE TABLE stats.rounds (
            ...
            created_at timestamptz NOT NULL DEFAULT NOW(),
            updated_at timestamptz NOT NULL DEFAULT NOW(),
            CONSTRAINT uq_rounds_player_id_tournament_id_round_number UNIQUE (player_id, tournament_id, round_number),
            CONSTRAINT ck_rounds_round_number CHECK (round_number BETWEEN 1 AND 8),
            CONSTRAINT fk_rounds_player_id_players FOREIGN KEY (player_id) REFERENCES stats.players(id) ON DELETE CASCADE,
            CONSTRAINT fk_rounds_tournament_id_tournaments FOREIGN KEY (tournament_id) REFERENCES stats.tournaments(id) ON DELETE CASCADE
        );
        ...
        CREATE TABLE analytics.projections (
            ...
            CONSTRAINT fk_projections_player_id_players FOREIGN KEY (player_id) REFERENCES stats.players(id) ON DELETE CASCADE,
            CONSTRAINT fk_projections_tournament_id_tournaments FOREIGN KEY (tournament_id) REFERENCES stats.tournaments(id) ON DELETE CASCADE
        );
        ...
        CREATE TABLE stats.betting_lines (
            ...
            CONSTRAINT fk_betting_lines_player_id_players FOREIGN KEY (player_id) REFERENCES stats.players(id) ON DELETE CASCADE,
            CONSTRAINT fk_betting_lines_tournament_id_tournaments FOREIGN KEY (tournament_id) REFERENCES stats.tournaments(id) ON DELETE CASCADE
        );
        """
    )
    ...
    op.execute(
        ...
        CREATE INDEX idx_articles_search ON content.articles
    ...
    op.execute(
        ...
        CREATE UNIQUE INDEX idx_player_recent_form_player_id
            ON analytics.player_recent_form (player_id);
        CREATE UNIQUE INDEX idx_leaderboard_summary_tournament_id_player_id
            ON analytics.leaderboard_summary (tournament_id, player_id);
        CREATE UNIQUE INDEX idx_projection_overview_projection_id
            ON analytics.projection_overview (projection_id);
        """
    )
